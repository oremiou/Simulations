...
##### unchecked STAR NETWORK - EQUAL EFFECTS FOR ALL TREATMENTS VS PLACEBO #
...
for (i in 1:N.sim)
{
  ...
  TE=c(rep(0,N.studies.comp*(N.treat-1))) 
  data1[[i]]=data.frame(TE,seTE)
  data1[[i]]$seTE=runif(N.studies.comp*(N.treat-1),0.9,1.1)
...
for (i in 1:N.sim)
{
  net1[[i]]=netmeta(TE=TE,comb.fixed=F, comb.random=T, seTE=seTE,treat1=t1, treat2=t2, studlab=studlab,data=data1[[i]]) 
}  
#netgraph(net1[[1]], plastic=F, multiarm = F, points=T, cex.points=5, col=1)  
